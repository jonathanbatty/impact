...
split_quoted_fields <- function(line) {
  quoted_fields <- regmatches(line, gregexpr('"[^"]*"', line))[[1]]
  substring(quoted_fields, 2L, nchar(quoted_fields) - 1L)
}

records <- lapply(synthetic_hes_rows, split_quoted_fields)
stopifnot(all(lengths(records) == length(columns)))
events <- as.data.frame(do.call(rbind, records), stringsAsFactors = FALSE)
names(events) <- columns
stopifnot(nrow(events) == 71L, !anyDuplicated(events$epikey))

diagnosis_columns <- grep("^diag_", names(events), value = TRUE)
operation_columns <- grep("^opertn_", names(events), value = TRUE)

# HES stores ICD-10 and OPCS-4 values without decimal points. IMPACT definitions
# use classification notation, so insert the decimal after the third character
# in a working copy while leaving the synthetic source records unchanged.
add_classification_decimal <- function(code) {
  ifelse(
    nchar(code) == 4L,
    paste0(substr(code, 1L, 3L), ".", substr(code, 4L, 4L)),
    code
  )
}
impact_events <- events
code_columns <- c(diagnosis_columns, operation_columns)
impact_events[code_columns] <- lapply(
  impact_events[code_columns],
...
admission_out <- impact(
  ...
  searchvars = list(diagnosis_columns, operation_columns),
  level = "phenotype",
  multimorbidity = TRUE,
  summary = TRUE
)
stopifnot(identical(admission_out$epikey, events$epikey))
admission_out <- cbind(events["hesid"], admission_out)
...
admission_counts <- admission_out[
  c("hesid", "epikey", "__nphenotypes", "__nmental", "__nphysical", "__nbody")
]
print(admission_counts, row.names = FALSE)
stopifnot(
  admission_out[["__nphenotypes"]][admission_out$epikey == "867770817181"] == 4L,
  admission_out[["__nphenotypes"]][admission_out$epikey == "708094304417"] == 7L,
  admission_out[["__nphenotypes"]][admission_out$epikey == "687864870336"] == 0L
)

phenotype_columns <- grep(
  "^__[A-Z0-9]{4}$", names(admission_out), value = TRUE
)
stopifnot(
  all(
    as.integer(rowSums(admission_out[phenotype_columns])) ==
      admission_out[["__nphenotypes"]]
  )
)
...
# across a person's admissions, then recalculate the grouped-phenotype count.
person_out <- aggregate(
  admission_out[phenotype_columns],
  by = list(hesid = admission_out$hesid),
  FUN = max
)
person_out[["__nphenotypes"]] <- as.integer(
  rowSums(person_out[phenotype_columns])
)
person_out[["multimorbidity"]] <- person_out[["__nphenotypes"]] >= 2L
person_out <- person_out[
  order(person_out$hesid),
  c("hesid", "__nphenotypes", "multimorbidity", phenotype_columns)
]
print(person_out[c("hesid", "__nphenotypes", "multimorbidity")], row.names = FALSE)

stopifnot(
  nrow(person_out) == 50L,
  person_out[["__nphenotypes"]][
    person_out$hesid == "4nzXgVgFrDjaI0oJCyFh6ewL2B20"
  ] == 13L,
  person_out[["__nphenotypes"]][
    person_out$hesid == "n9E2gPm0og60e2vm3iyPfPlPlVLm"
  ] == 11L,
  !person_out[["multimorbidity"]][
    person_out$hesid == "IXWlaOLJlmBjo0CxShd6z3jTBZDc"
  ]
)

message("OK: IMPACT R HES APC example passed")
