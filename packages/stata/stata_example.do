...
input str32 hesid str12 epikey str4(diag_01 diag_02 diag_03 diag_04 diag_05 diag_06 diag_07 diag_08 diag_09 diag_10 opertn_01 opertn_02 opertn_03 opertn_04 opertn_05)
...
unab code_vars : diag_* opertn_*
recast str5 `code_vars'
foreach variable of local code_vars {
    replace `variable' = substr(`variable', 1, 3) + "." + ///
        substr(`variable', 4, 1) if strlen(`variable') == 4
}

tempfile events admissions
save `events'
preserve
keep hesid epikey
save `admissions'
...
* Apply ICD-10 to diagnosis fields and OPCS-4 to procedure fields in one run.
clear
impact, dataset(`events') id(epikey) codesystems(icd10 opcs4) ///
        searchvars(diag_* opertn_*) level(phenotype) multimorbidity summary

* Restore the person identifier and inspect phenotype counts for each admission.
merge 1:1 epikey using `admissions', assert(match) nogen
order hesid epikey
sort hesid epikey
list hesid epikey __nphenotypes __nmental __nphysical __nbody, ///
    noobs abbreviate(32)
assert __nphenotypes == 4 if epikey == "867770817181"
assert __nphenotypes == 7 if epikey == "708094304417"
assert __nphenotypes == 0 if epikey == "687864870336"
...
keep hesid `phenotype_vars'
collapse (max) `phenotype_vars', by(hesid)
egen int __nphenotypes = rowtotal(`phenotype_vars')
generate byte multimorbidity = __nphenotypes >= 2
label variable multimorbidity "Two or more grouped phenotypes"
sort hesid
list hesid __nphenotypes multimorbidity, noobs abbreviate(32)
assert __nphenotypes == 13 if hesid == "4nzXgVgFrDjaI0oJCyFh6ewL2B20"
assert __nphenotypes == 11 if hesid == "n9E2gPm0og60e2vm3iyPfPlPlVLm"
assert multimorbidity == 0 if hesid == "IXWlaOLJlmBjo0CxShd6z3jTBZDc"
assert _N == 50

display as result "OK: IMPACT Stata HES APC example passed"
